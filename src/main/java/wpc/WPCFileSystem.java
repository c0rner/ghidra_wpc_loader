...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = WPCFileSystem.MyFileSystemFactory.class)
public class WPCFileSystem implements GFileSystem {
	 ...
	 */
	public WPCFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + WPCFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<WPCFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public WPCFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			WPCFileSystem fs = new WPCFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
